...
const Navigation = ()=>{
    ...
    return (
        <header className = {styl}>
            ...
            <nav className={`${classess.flex} ${classess['nav-size']} ${classess['flex-evenly']} ${classess['nav-style']} ${classess['box-position-absolute']}`}>      
                <a href="http://localhost:5000">Home    
                </a>
                <div className={`${classess['flex']} ${classess['flex-center']}`} onMouseEnter={mouseEnterHandler} >
                <a href="http://localhost:5000/category" className={`${classess['nav-category-modal']}`} >Category</a>

            <div className={` ${classess['box-position']} ${(!isHover)?classess['hidden']:''}` } onMouseLeave ={mouseLeaveHandler}>
            <ul>
                    <li><a href="http://localhost:5000/men">Men</a></li>
                    <li><a href="http://localhost:5000/women">Women</a></li>
                </ul>
            </div>
                </div>
